...
import { Folder, Trash2, Download, FileImage, Video, Music, FileText, HardDrive } from "lucide-react";
...
export default function RealStorageCleaner() {
    ...
    try {
      ...
      if (!('showDirectoryPicker' in window)) {
        toast({
          ...
          description: "Please use Chrome or Edge browser with HTTPS for folder scanning",
          variant: "destructive",
        });
        return;
      }

      // Check if we're on HTTPS (required for File System Access API)
      if (location.protocol !== 'https:' && location.hostname !== 'localhost') {
        toast({
          title: "HTTPS Required",
          description: "Real folder scanning requires a secure connection. Please use the custom domain swipeeclean.com",
          variant: "destructive",
        });
        return;
      }
        ...
        for await (const [name, handle] of dirHandle.entries()) {
          if (handle.kind === 'file') {
            try {
              ...
              setProgress(files.length * 2); // Rough progress estimate
            } catch (error) {
      ...
      toast({
        title: "Scan Complete",
        description: `Found ${files.length} files (${formatFileSize(totalSize)})`,
      });
      ...
      if (error.name !== 'AbortError') {
        toast({
          title: "Scan Failed",
          description: "Unable to access folder. Please try again.",
          variant: "destructive",
        });
      }
    } finally {
  ...
  const deleteSelectedFiles = async () => {
    ...
    try {
      // Note: Actual file deletion through File System Access API requires additional permissions
      // For now, we'll simulate the deletion and show the space that would be saved
      ...
      toast({
        ...
        description: `Would delete ${deletedCount} files (${formatFileSize(deletedSize)})`,
      });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="text-center">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Real Storage Cleaner</h1>
        <p className="text-gray-600">Scan and clean actual files on your device</p>
      </div>
      ...
      {!analysis && (
        ...
        >
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
            ...
            <h3 className="text-lg font-semibold mb-2">Select Folder to Scan</h3>
            <p className="text-gray-600 mb-4">
              Choose a folder to analyze for duplicates, old files, and space optimization
            </p>
            <Button 
              onClick={scanFolder} 
              disabled={isScanning}
          ...
          {selectedFiles.size > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <div className="flex justify-between items-center">
                ...
                >
                  {isDeleting ? 'Cleaning...' : 'Clean Selected Files'}
                </Button>
              </div>
            </div>
          )}
